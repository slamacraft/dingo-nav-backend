...
class PackageScanner(var classLoader: ClassLoader) {
  ...
  implicit class PackageWalker(val packageName: String) {
    ...
    def walkClass(filter: Class[_] => Boolean = _ => true): mutable.MutableList[Class[_]] = {
      val result = mutable.MutableList[Class[_]]()
  ...
  def scanLocalClasses(packagePath: String): Unit = {
    classes ++= packagePath.walkClass()
  }

  def scanJarClasses(packagePath: String): Unit = {
