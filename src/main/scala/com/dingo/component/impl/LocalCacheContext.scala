package com.dingo.component.impl

import com.dingo.common.JsonMapper
import com.dingo.common.util.RandomUtil
...
class LocalCacheContext extends ICacheContext {
  private val cacheMap = new mutable.HashMap[String, Any]()
  ...
  override def remove[T](key: String)(implicit tag: ClassTag[T]): Option[T] = {
    cacheMap.get(key).map { it =>
      ...
      val it: Option[T] = getExpireCache(key)(tag)
